...
async function requestJson(path, { signal, method = "GET", body } = {}) {
    ...
    if (hasNetworkError) {
        throw new Error(`Could not connect to caregiver messages API on http://localhost:4001. Start API server and retry. Details: ${errors.join(" | ")}`);
    }

    throw new Error(`Caregiver messages API request failed. ${errors.join(" | ")}`);
}
...
export async function markCaregiverMessageRead(messageId, { signal } = {}) {
    if (!messageId) {
        throw new Error("Message id is required.");
    }

    return requestJson(`/api/v1/caregiver/messages/${encodeURIComponent(String(messageId))}/read`, {
        signal,
        method: "PUT"
    });
}

export async function getCaregiverConversations({ signal, page = 1, limit = 50 } = {}) {
    const query = new URLSearchParams({ page: String(page), limit: String(limit) }).toString();
    const payload = await requestJson(`/api/v1/caregiver/conversations?${query}`, { signal });
...
export async function getCaregiverConversationMessages(conversationId, { signal, page = 1, limit = 100 } = {}) {
    if (!conversationId) {
        throw new Error("Conversation id is required.");
    }

    const query = new URLSearchParams({ page: String(page), limit: String(limit) }).toString();
    const payload = await requestJson(`/api/v1/caregiver/conversations/${conversationId}/messages?${query}`, { signal });
...
export async function sendCaregiverConversationMessage(conversationId, text, { signal } = {}) {
    if (!conversationId) {
        throw new Error("Conversation id is required.");
    }
    ...
    return requestJson(`/api/v1/caregiver/conversations/${conversationId}/messages`, {
        signal,
        method: "POST",
        body: { text: cleanText }
    });
